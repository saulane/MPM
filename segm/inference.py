...
ADE20K_CATS_PATH = Path(__file__).parent / "data" / "config" / "ade20k.yml"
...
@click.option("--mpm-layers", default=None, type=str, help="Comma-separated MPM insertion layers, e.g. 2,5.")
def main(model_path, input_dir, output_dir, gpu, mpm_layers):
    ptu.set_gpu_mode(gpu)

    model_dir = Path(model_path).parent
    model, variant = load_model(model_path)
    if mpm_layers:
        layers = [int(x) for x in mpm_layers.replace(",", " ").split()]
        model.encoder.global_merge = True
        model.encoder.mpm_layers = layers
    model.to(ptu.device)
